...
A bundled custom card (`alexa-shopping-categoriser-card`) renders the projection and drives the
...
- Render the shop → category (aisle) → items tree in order (`No Preference` shop last;
  `Uncategorised` category last within each shop). Collapse/de-emphasize groups with zero unchecked
...
- Category settings panel → reads the `category_definitions` sensor attribute to display
  ...
  (`add/edit/delete_category`, `recategorise_item`).
- **Shop preference (Req 7):** the card renders the projection as a **two-level nested tree**:
  **shop → category (aisle) → items** (from `shop_groups`, `No Preference` last, `Uncategorised`
...
- **First-setup review affordance (Req 1.7 intent, finding M-1):** on first setup the card shows
  ...
  prominently-surfaced `Uncategorised` bucket, giving the user a non-blocking review opportunity
...
- **Optimistic placement is a best-effort guess (finding F4-7):** for a just-typed item the card
  predicts both shop and category from the text via the pure categoriser, but it cannot see a
  backend-only learned override (`recategorise_item`/`assign_shop`) until the inbound refresh. So a
...
- `entity` (the categorised sensor) — required.
...
| Move item's category | `alexa_shopping_categoriser.recategorise_item` (item_text, category, apply_to_uid) |
| Set item's shop | `alexa_shopping_categoriser.assign_shop` (item_text, shop, apply_to_uid); `No Preference` clears it |
