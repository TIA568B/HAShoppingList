...
- **Files:** `custom_components/alexa_shopping_categoriser/{__init__,const,manifest.json}`,
...
## Phase 1 — Pure categoriser + models + store
- **Objective:** the deterministic categorisation + shop-resolution core + persistence.
- **Files:** `models.py`, `categoriser.py`, `store.py`, default taxonomy + default shops.
...
- **Acceptance:** categoriser passes the full vegan matrix **and the shop-resolution matrix**
...
- **Tests:** `test_categoriser.py` (categories + shops), store/migration tests.
...
- **Acceptance:** selecting the `alexa_devices` todo list creates an entry; sensor shows the
  categorised projection matching the contract (v3: shop-primary `shop_groups`,
...
- **Objective:** category + shop CRUD + learned re-categorisation/assignment.
- **Files:** `services.py`, `services.yaml`, options flow additions.
- **Dependencies:** Phase 2.
- **Acceptance:** category services as before (add/edit/delete + `recategorise_item`, delete→
  Uncategorised, rename migrates overrides); **shop services** `assign_shop` (learns; `No
...
- **Acceptance (from spec tasks 5.3–5.6):** add via Alexa voice appears categorised in seconds;
...
proceed in parallel with backend wiring since the categoriser and the card's timer logic are
