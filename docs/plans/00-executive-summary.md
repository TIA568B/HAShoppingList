...
(Produce, Milk, Chilled, Fake Meat, Bakery, Frozen, Drinks, Pantry, Household, Uncategorised),
with **reactive live updates**, **optimistic tick-off with a per-item undo grace period**, and
**two-way sync** back to the Alexa list. Categorisation is seeded from a vegan-appropriate
...
for mis-taps. The user is vegan, so the categorisation must avoid animal-derived categories and
...
flowchart TD
    ...
    SRC -->|state_changed event| COORD[alexa_shopping_categoriser coordinator]
    STORE[(category map + learned overrides<br/>HA Store)] --> COORD
    COORD --> CAT[categoriser pure functions]
    CAT --> COORD
    COORD --> SENSOR["sensor.*_categorised<br/>(derived projection)"]
    SENSOR -->|websocket subscribe| CARD[Custom Lovelace card]
    CARD -->|todo.update_item / todo.add_item| SRC
    CARD -->|recategorise_item etc.| SVC[integration services]
    SVC --> STORE
```
...
- **Inbound:** source list changes (from Alexa or anywhere) → coordinator recomputes the
  categorised projection → sensor updates → card re-renders.
...
A **custom, config-flow-based Home Assistant integration** (`alexa_shopping_categoriser`),
...
- **No history mining.** The `alexa_devices` todo entity retains completed items and exposes
  them via `todo.get_items`; that live corpus (active + completed) seeds categorisation. HA
